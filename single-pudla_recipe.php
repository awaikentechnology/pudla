...
$page_layout	=	get_theme_mod( 'recipe_single_page_layout', 'with-sidebar' );
...
<section class="page-single">
	...
	<?php while ( have_posts() ) : the_post(); ?>
		<?php if( true == get_theme_mod( 'show_recipe_fullwidth_title', true ) ): ?>
			<?php get_template_part( 'templates/single-recipe', 'title' ); ?>
		<?php endif; ?>
		<div class="row">
			<div class="<?php echo esc_attr( $column ); ?>">
				<?php if( false == get_theme_mod( 'show_recipe_fullwidth_title', true ) ): ?>
					<?php get_template_part( 'templates/single-recipe', 'title' ); ?>
				<?php endif; ?>
				<div class="post-entry">
				<?php if( true == get_theme_mod( 'show_recipe_header', true ) ): ?>
					<?php 
					...
					<?php }else{ ?>
						...
						<div class="single-post-header post-image-header">
							<img itemprop="image" src="<?php the_post_thumbnail_url( 'full' ); ?>"/>
						</div>
						<?php endif; ?>
					<?php } ?>
					<?php endif; /* show recipe header */ ?>
					...
					?>
					<?php if( true == get_theme_mod( 'show_recipe_tags', true ) ): ?>
						<?php 
							$recipe_tag    = get_the_terms( get_the_ID(), 'recipe_tag' );
							if($recipe_tag){ 
							$out = array();
						?>
						<span class="tag-list"><b><?php esc_html_e( 'Tags:', 'pudla' ); ?></b>
						<?php 
							foreach ( $recipe_tag as $item_cat ) {
								$out[] = '<a rel="tag" href="' . esc_url( get_category_link( $item_cat->term_id ) ) . '">' . $item_cat->name . '</a>';
							}
							echo implode( ', ', $out );
						?>
						</span>
					...
					<?php if( true == get_theme_mod( 'show_recipe_cuisine', true ) ): ?>
						<?php 
							$recipe_cuisine    = get_the_terms( get_the_ID(), 'recipe_cuisine' );
							if($recipe_cuisine){ 
							$out = array();
						?>
						<span class="tag-list"><b><?php esc_html_e( 'Cuisine:', 'pudla' ); ?></b>
							<?php 
								foreach ( $recipe_cuisine as $item_cat ) {
									$out[] = '<a rel="tag" href="' . esc_url( get_category_link( $item_cat->term_id ) ) . '">' . $item_cat->name . '</a>';
								}
								echo implode( ', ', $out );
							?>
					...
					<?php 
					if( true == get_theme_mod( 'show_recipe_author_info', true ) ): 
				...
				<?php $prev_post = get_adjacent_post( true, '', true, 'recipe_category' ); ?>
				<?php $next_post = get_adjacent_post( true, '', false, 'recipe_category' ); ?>
